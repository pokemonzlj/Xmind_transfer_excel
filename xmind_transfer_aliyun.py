...
"""适用于阿里云效的用例导入，xmind形式编写用例后，自动生成阿里云效导入格式的用例"""
...
def xmind_cat(lst, file_name='测试用例'):
    ...
    title_list = ['目录', '标题', '优先级', '前置条件', '步骤描述', '预期结果', '类型', '用例状态', '负责人', '关联需求']
    ...
    for h in range(len(lst)):
        ...
        for j in range(len(lists)):
            ...
            # print(lists[j])
            start_column = 1
            module_path = "|".join(lists[j][0:2])
            if 7 > len(lists[j]) >= 5:
                module_details = '-'.join(lists[j][2:5])
                sheet.cell(row=j + index + 1, column=1).value = module_path
                ...
                if module_details == prev_module_details:
                    index -= 1
                    if lists[j][6].startswith('前置条件'):
                        sub_preconditions = clean_precondition(lists[j][6], "前置条件")
                        sheet.cell(row=j + index + 1, column=4).value = sub_preconditions

                    elif lists[j][6].startswith('步骤'):
                        sub_steps = clean_precondition(lists[j][6], "步骤")
                        sheet.cell(row=j + index + 1, column=5).value = sub_steps

                    elif lists[j][6].startswith('预期'):
                        sub_expected = clean_precondition(lists[j][6], "预期")
    ...
    filename = f"测试用例/{date_time}_{file_name}_case.xlsx"
    wb.save(filename)
...
def maintest():
    ...
    xmind_cat([out[0]['topic']], file_name)
